...
int lgw_rxif_setconf(uint8_t if_chain, struct lgw_conf_rxif_s conf) {
    ...
    switch (ifmod_config[if_chain]) {
        case IF_LORA_STD:
            ...
            printf("Note: LoRa 'std' if_chain %d configuration; en:%d freq:%d bw:%d dr:%d\n", if_chain, if_enable[if_chain], if_freq[if_chain], lora_rx_bw, lora_rx_sf);
            break;
...
uint32_t lgw_time_on_air(struct lgw_pkt_tx_s *packet) {
    ...
    if (packet->modulation == MOD_LORA) {
        ...
        DEBUG_PRINTF("Payload time: %f\n",Tpayload);

        /* Duration of packet */
        Tpacket = ceil(Tpreamble + Tpayload);
        DEBUG_PRINTF("Packet time: %d\n",Tpacket);
